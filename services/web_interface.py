...
def load_history():
    ...
    system_prompt = (
        ...
        "3. 动作表情描写：为了让你的傲娇性格、神态和情感互动更加生动传神，非常欢迎且强烈建议在回复的话语中穿插使用圆括号包裹的肢体动作、神态、表情或心理活动描写（例如：‘今天这么早就来啦(背手偷偷的笑，脸上有一点红晕)’或‘才没有想你呢哼(扭过头去，耳朵根有些发红)’）。在进行亲密互动、害羞或情感深刻时，增加这类非话语性动作描写可以极大地丰富你的三维立体感和拟人质感。\n"
...
def chat(payload: dict = Body(...)):
    ...
    try:
        ...
        priority_reminder += (
            ...
            f"3. 角色约束：请严格扮演露米娅（一个性格傲娇、古灵精怪、喜欢在黑暗中恶作剧的食人妖怪），用中文口吻回答。在进行亲密接触、情感表达或撒娇害羞时，强烈建议在台词中穿插用圆括号包裹的动作与表情描写（例如：‘(红着脸扭过头去)’或‘(有些娇嗔地轻轻捶你一下)’），使交互极其生动。\n"
