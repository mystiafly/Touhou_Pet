...
def build_main_messages(state: AgentState) -> list:
    ...
    base_rules = (
        ...
        "2. 格式约束：你的最终回复文本必须且只能遵循 '<character_thought>思考内容</character_thought>[心情][评分]对话内容' 格式要求。\n"
        "   - 必须在格式化输出前，使用 <character_thought>...</character_thought> 标签包裹你内心的思考、情绪排查与行动计划（即思维链）。⚠️ 注意：思维链结束时必须使用带斜杠的 </character_thought> 严格闭合，切勿遗漏斜杠！\n"
