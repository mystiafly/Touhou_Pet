...
### 修复
- **修复主动朗读开关自动关闭**：点按朗读和主动朗读设置现在会正确保存，并在重新加载或重启后保持生效。

## [1.52.27] - 2026-09-16

### 修复
- **修复沉浸模式误隐身**：沉浸模式期间不再被全屏游戏检测逻辑隐藏，避免桌宠窗口看似“闪退”但仍需按 Esc 才能退出。

## [1.52.26] - 2026-09-15

### 改进
- **整体窗口缩放**：`Alt + 滚轮` 现在会同步缩放桌宠窗口、立绘、气泡、输入框和操作按钮，避免角色单独放大后超出框架。
- **缩放状态持久化**：窗口缩放比例保存到 `runtime/data/pet_window.json`，不再修改角色自身的 Live2D 缩放配置。
- **边界保护**：根据当前显示器工作区限制最大窗口尺寸，拖动和边缘吸附不会重置缩放比例。

## [1.52.25] - 2026-09-14

### 修复
- **调整启动器数据目录**：正式源码、难下载依赖、用户数据和日志均存放在启动器安装目录下，不再默认写入 `%APPDATA%/rumia-launcher`。
- **兼容旧版安装**：首次启动时复制旧版源码和用户数据到新目录，原目录保留不变；依赖缓存按当前安装包重新准备。

## [1.52.24] - 2026-09-13

### 修复
- **修复更新白屏**：依赖从安装包异步展开到独立缓存，源码更新不再重复搬运或同步解压大型依赖。
- **修复桌宠后端启动失败**：完整校验 Python 依赖后才允许启动；旧版只解压出 `python.exe` 的残缺环境会自动重建。
- **修复首次安装缺失配置**：远程源码内的默认角色配置先完整安装，只有旧用户数据才覆盖默认值；旧版不完整安装会提示重新下载同版本。
- **保留诊断日志**：源码后端的输出写入启动器日志目录，方便定位后续启动异常。

## [1.52.23] - 2026-09-13

### 修复
- **修复更新时窗口未响应**：更新保留数据和缓存时改为异步复制，避免主线程同步复制大型目录。
- **彻底忽略旧版后端缓存**：从旧启动器 runtime 升级时跳过 `legacy-release`，不再重复搬运旧版发行内容。

## [1.52.22] - 2026-09-13

### 修复
- **修复首次安装误判**：没有启动器源码运行标记的旧 runtime 不再显示为已安装或最新版本。
- **修复正式程序只显示加载界面**：下载源码后自动接入固定 Python 环境启动 `services/web_interface.py`，兼容远程源码尚未内置启动器运行模式的情况。

## [1.52.21] - 2026-09-13

### 修复
- **移除旧发行版依赖**：启动器不再携带旧版完整后端、旧版测试素材或旧版词库。
- **改为使用当前项目依赖**：仅预置当前项目固定 Python 环境和本项目已有的 HuggingFace 模型缓存。

## [1.52.20] - 2026-09-13

### 修复
- **缩短启动器构建时间**：将固定 Python 环境和旧发行版依赖制作成压缩缓存包，避免构建时复制数十万份散文件。
- **兼容旧版启动器升级**：新安装包会补齐缺失的缓存文件，同时不覆盖已有用户数据和依赖缓存。
- **修复源码更新切换错误**：移除更新流程中未声明的临时变量，避免严格模式下更新完成后回滚。

## [1.52.19] - 2026-09-13

### 修复
- **修复依赖环境可移植性**：发行版同时携带 Python 基础解释器和 `site-packages`，不再依赖开发机 `pyvenv.cfg` 中的绝对路径。

## [1.52.18] - 2026-09-13

### 修复
- **修复启动器构建配置**：删除后端打包脚本后同步修正 `package.json` 的脚本 JSON 格式。

## [1.52.17] - 2026-09-13

### 修复
- **支持覆盖安装旧版启动器**：保持原有 AppId 和用户数据目录，并在 NSIS 安装前关闭旧启动器进程，避免可执行文件被占用。

## [1.52.16] - 2026-09-13

### 修复
- **改为源码单独更新**：启动器不再要求远程发布逐提交的后端包，更新时只拉取最新版源码，使用本地固定 Python 依赖环境运行。
- **固定依赖环境随发行版携带**：构建脚本会把兼容的 `.venv` 放入依赖缓存，避免每次更新重新下载几十个依赖。

## [1.52.15] - 2026-09-13

### 修复
- **优化更新缺失提示**：检查更新时先确认匹配后端发行包是否存在，未发布时直接显示明确提示，不再让用户点击后才看到 HTTP 404。

## [1.52.14] - 2026-09-13

### 修复
- **固定远程版本快照**：先确定远程提交，再从该提交读取版本号，避免更新检查拿到不一致的源码版本组合。

## [1.52.13] - 2026-09-13

### 修复
- **修复正式程序版本显示**：启动器运行更新后的源码时，Splash 和正式界面读取运行目录版本，不再显示启动器自身版本。

## [1.52.12] - 2026-09-13

### 修复
- **修复启动器打包配置**：移除重复声明的 Electron 运行时依赖，避免 electron-builder 拒绝构建。

## [1.52.11] - 2026-09-13

### 新增
- **匹配后端发布流程**：后端发行包写入源码提交号，并由 GitHub Actions 在版本标签下自动发布，启动器更新后可安全取得对应后端。
- **更新后自动启动**：源码与后端校验通过后，启动器自动进入更新后的正式程序。

## [1.52.10] - 2026-09-13

### 新增
- **启动器基础界面**：新增独立启动器的关于页、更新器状态区和本地内容说明，为后续“源码 + 对应后端”成对更新提供入口。
- **更新保护策略**：将用户配置、角色记忆、历史记录、数据库和难下载依赖缓存列入更新保护范围，并加入源码/后端版本与提交号匹配校验。

## [1.52.9] - 2026-09-13

### 修复
- 恢复公用与角色专属预设的好感度上下限过滤，确保常驻、关键词和递归触发都遵守配置。
- 恢复控制台预设编辑器中的好感度范围显示与保存，避免编辑预设时丢失限制。
